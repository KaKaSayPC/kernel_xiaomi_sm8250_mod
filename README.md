...
English
This repo (android15-lineage22-mod branch) is based on Lineage OS 22.1 xiaomi sm8250 kernel source.

Originally this repo (android12-stable-mod or android14-stable-mod branch) is fork from UtsavBalar1231's repo, but the android14-stable branch has freeze problem when the device sleep for a while or wake up. So now I switched to lineage22 codebase. The MIUI features code and some parts of the drivers is still copied from UtsavBalar1231's branch.

So still Thanks to @UtsavBalar1231!

The main purpose of maintaining and building this kernel is to fix this battery stuck at 1% problem, and provide SukiSU(A KernelSU fork with KPM support) & SUSFS integrated pre-built image(flashable anykernel3 zip). Also provides a more intuitive and easy-to-use build script and build guide that allow you to try to build by yourself.

For using the SukiSU, you can install the SukiSU manager APK from SukiSU Github Release. For the SUSFS module see susfs4ksu-module Gihub Release.
...
中文
该repo (android15-lineage22-mod 分支)主要基于Lineage OS 22.1 xiaomi sm8250 kernel source。

原来这个repo(即android12-stable-mod/android14-stable-mod分支)是fork自UtsavBalar1231的仓库，但切到android14-stable分支的时候，发现那套代码有睡死问题（202408的几个release），所以现在切到了基于lineage22的代码来搞。MIUI特性的代码以及部分的设备驱动抠自UtsavBalar1231的仓库。

所以仍然感谢 @UtsavBalar1231！

维护和编译这个内核的主要目的是想修复电量卡在1%的问题，以及提供带SukiSU(一个KernelSU的fork，支持KPM) & SUSFS的预编译好的内核（请自行安装SukiSU的管理器以及根据需要刷上SUSFS模块）。以及再提供一个更直观和易用的编译脚本和README，方便大家自己折腾和修改，编译自己的内核！

（其中受“1%电量bug”影响的设备有：alioth, apollo, lmi, thyme, umi, pipa，因为它们都用了PM8150即高通的GEN4电量计。其它不受此bug影响的设备大可把这个内核当成个带SukiSU & SUSFS的官核平替，如果你想找一个带KernelSU的内核的话。并且据大家测试，该内核不带KernelSU版本可以应用APatch）

Release里的编译好的内核成品由android15-lineage22-mod分支编译，应当能在原版MIUI和第三方的基于AOSP的各种Android11-15的ROM上使用。欢迎大家尝试并反馈（提Issue或Pull Requests）！酷友们到酷安的这个帖子讨论或反馈，也可以给我私信反馈！
...
支持 CANBus 和 USB CAN （如 CANable）适配器（一些折腾嵌入式的可能会喜欢这个）
zRAM 支持 LZ4、LZ4HC、ZSTD 压缩算法
How to build
Prepair the basic build environment.

You have to have the basic common toolchains, such as git, make, curl, bison, flex, zip, etc, and some other packages. In Debian/Ubuntu, you can
...
Download [proton-clang] compiler toolchain

You have to have aarch64-linux-gnu, arm-linux-gnueabi, clang. Proton Clang is a good prebuilt clang cross compiler toolchain.
